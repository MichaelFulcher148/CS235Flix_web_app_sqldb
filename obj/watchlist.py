...
class WatchList:
    ...
    def __next__(self):
        if self.__current == len(self._the_list):
            raise StopIteration
        else:
            data = self._the_list[self.__current]
            self.__current += 1
            return data
    ...
    def first_movie_in_watchlist(self) -> 'Movie' or None:
        if len(self._the_list) != 0:
            return self._the_list[0]
        return None

    def add_movie(self, a_movie: 'Movie') -> None:
        if isinstance(a_movie, Movie):
            if a_movie not in self._the_list:
                self._the_list.append(a_movie)
        else:
            raise TypeError
    ...
    def remove_movie(self, a_movie: 'Movie') -> None:
        if isinstance(a_movie, Movie):
            try:
                num = self._the_list.index(a_movie)
            except ValueError:
                return
            self._the_list.pop(num)
        else:
            raise TypeError

    def select_movie_to_watch(self, num: int) -> 'Movie' or None:
        if num + 1 > len(self._the_list):
            return None
        return self._the_list[num]
